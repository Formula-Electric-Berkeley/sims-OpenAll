...
% single value vars currently work (no motor curves).
sweepVar = "Cl";

% Values to sweep var with
vals2Sweep = [-1:-0.25:-2];

% Sweep second var? Set True if yes
sweep2 = false;

% Second var to sweep
sweepVar2 = "Cd";

% Values to sweep second var. USE LINSPACE and fill out values (need same
% length as vals2Sweep
vals2Sweep2 = linspace(0,-1,length(vals2Sweep));
...
if length(vals2Sweep) == 1 && (length(vals2Sweep2) == 1 || ~sweep2)
    ...
    [sim] = simulate(veh,tr);
    t = sim.laptime.data;
    e = sim.energy.data(end-1)-sim.regen.data(end-1);
    
    %% Reporting results if single value
    display("Laptime: " + t + " seconds")
    display("Enduranc Energy with Regen: " + e + " kWh")
else 
    if ~sweep2
        %Sweeping 1 variable
    
        %Initializing  lap time, and endurance energy vectors
        tVec  = zeros(length(vals2Sweep),1);
        eVec  = zeros(length(vals2Sweep),1);
        
        %Run sim for each sweep value and record time and energy
        for i = 1:length(vals2Sweep); 
            veh.(sweepVar) = vals2Sweep(i);
            [sim] = simulate(veh,tr);
            tVec(i) = sim.laptime.data;
            eVec(i) = sim.energy.data(end-1)-sim.regen.data(end-1);
        end
        
        %Plot time and erergy versus swept value
        figure;
        subplot(2,1,1)
        plot(vals2Sweep, tVec, "O-");
        title("Lap Time Vs. Swept " + sweepVar)
        xlabel(sweepVar)
        ylabel("Lap Time (s)")
        grid on;
        
        subplot(2,1,2)
        plot(vals2Sweep, eVec, "O-");
        title("Endurance Energy Vs. Swept " + sweepVar)
        xlabel(sweepVar)
        ylabel("Endurance Energy w/ Regen (kWh)")
        grid on;
    else
        %Sweeping 2 variables
        
        %Initializing  lap time, and endurance energy vectors
        tVec  = zeros(length(vals2Sweep),length(vals2Sweep2));
        eVec  = zeros(length(vals2Sweep),length(vals2Sweep2));
        ...
        for i = 1:length(vals2Sweep); 
            for j = 1:length(vals2Sweep2)
                ...
                [sim] = simulate(veh,tr);
                tVec(i,j) = sim.laptime.data;
                eVec(i,j) = sim.energy.data(end-1)-sim.regen.data(end-1);
            end
        end
        ...
        figure;
        subplot(2,1,1)
        surf(vals2Sweep, vals2Sweep2, tVec);
        title("Lap Time Vs. Swept " + sweepVar)
        xlabel(sweepVar)
        ylabel(sweepVar2)
        zlabel("Lap Time (s)")
        ...
        subplot(2,1,2)
        surf(vals2Sweep2, vals2Sweep2, eVec);
        title("Endurance Energy Vs. Swept " + sweepVar)
        xlabel(sweepVar)
        ylabel(sweepVar2)
        zlabel("Endurance Energy w/ Regen (kWh)")
        colorbar
    end
end
...
function [sim] = simulate(veh,tr)
    ...
    energy = cumtrapz(time, engine_power)*2.77778e-7*22/tr_length;
